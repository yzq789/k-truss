...
//	my_timer.end();
    ...
    communities->clear();
    communities = query_processor->queryKTrussCommunities(9, 5);
    display(communities);
//    query_processor->displayKTruss();
    delete query_processor;

	char *test_path2 = ROOT_PATH"data/test2.txt";
	query_processor = new QueryProcessor(test_path2);
	query_processor->deleteEdge(32, 42);
    communities->clear();
	communities = query_processor->queryKTrussCommunities(9, 5);
	display(communities);
    query_processor->displayKTruss();
...
//    communities->clear();
//	communities = query_processor->queryKTrussCommunities(414, 20);
//    display(communities);
